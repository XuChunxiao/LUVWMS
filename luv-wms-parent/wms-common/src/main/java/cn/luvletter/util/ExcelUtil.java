...
public class ExcelUtil {
    ...
    public static void main(String[] args) {  
        ...
        String src_xlspath = "C:\\Users\\72659\\Desktop\\入库单导入.xlsx";
        ...
        for (Row row : rowList) {
			System.out.println("row:"+ getCellValue(row.getCell(0)));
		}
    }  
        ...
        try {  
            ...
            if("xls".equals(ext)){              //使用xls方式写入  
                ...
                try{  
                    ...
                    logger.error("您要操作的文件没有扩展名，正在尝试以xls方式写入错误",e1);
    ...
    public void writeExcel_xls(List<Row> rowList, String src_xlsPath, String dist_xlsPath) throws IOException {  
        ...
        try {  
            ...
            logger.error("将rowlist的内容写到Excel中错误",e);
        }  
    }  
    ...
    public void writeExcel_xlsx(List<Row> rowList, String src_xlsPath, String dist_xlsPath) throws IOException {  
        ...
        try {  
            ...
            logger.error("将rowlist的内容添加到Excel中错误",e);
        }  
    }  
    ...
    public List<Row> readExcel_xlsx(String xlsPath) throws IOException {  
        ...
        try {  
            ...
            logger.error("读取Excel 2007版，xlsx格式",e);
        }  
        return rowList;  
    }  
...
//            }  
            ...
            for (int i = startReadPos; i <= lastRowNum + endReadPos; i++) {  
                ...
                // 第一个单元格如果为空，此处认为excel结束
                if (row != null && (!getCellValue(row.getCell(0)).equals("") || !getCellValue(row.getCell(1)).equals(""))) {  
                    rowList.add(row);  
                     // 获取每一单元格的值  
